...
- Member/method access uses `->`: `vec->push_back(x).`, `vec->size`
- Vector type is under `sti`: declare as `sti<->vector<-int32 v.`; `import sti.` is sufficient (no separate `import vector.` needed)
- Vector methods: `->push_back(x)`, `->pop_back()`, `->clear()`, `->sort()` (ascending insertion sort, emitted inline)
- Extern C functions: `extern int32 myFunc(int32 x).` — declares a signature, body provided by the linker
